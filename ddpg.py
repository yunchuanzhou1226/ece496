...
class ReplayBuffer:
    ...
    def __init__(self, CktGraph, size: int, batch_size: int = 32):
        ...
        """Initializate."""
        self.obs_buf = np.zeros([size, self.num_nodes, self.num_node_features], dtype=np.float32)
        self.next_obs_buf = np.zeros([size, self.num_nodes, self.num_node_features], dtype=np.float32)
...
class DDPGAgent:
        ...
        if self.is_test == False:
            ...
            if self.total_step < self.initial_random_steps: # if initial random action should be conducted
                ...
                if self.noise_type == 'uniform':
                    ...
                    selected_action = np.random.uniform(np.clip(selected_action-self.noise_sigma, -1, 1), 
                                                        np.clip(selected_action+self.noise_sigma, -1, 1))

                self.noise_sigma = max(self.noise_sigma_min, self.noise_sigma*self.noise_sigma_decay)
    ...
    def train(self, num_steps: int, step_save: bool = False, plotting_interval: int = 10):
        """Train the agent."""
        self.is_test = False
        ...
        for self.total_step in range(1, num_steps + 1):
            ...
            if step_save:
                ...
                if self.total_step in save_list:
                    ...
                    save_name_actor = f"Actor_{date}_reward={best_reward:.2f}_step={self.total_step}.pth"
                    
                    model_weight_critic = self.critic.state_dict()
                    save_name_critic = f"Critic_{date}_reward={best_reward:.2f}_step={self.total_step}.pth"
                    
                    torch.save(model_weight_actor, save_path.joinpath("saved_weights/" + save_name_actor))
                    torch.save(model_weight_critic, save_path.joinpath("saved_weights/" + save_name_critic))
                    print("Actor and Critic weights have been saved!")

                    # save memory
                    with open(save_path.joinpath(f'saved_memories/memory_{date}_reward={best_reward:.2f}_step={self.total_step}.pkl'), 'wb') as memory_file:
                        pickle.dump(self.memory, memory_file)

                    np.save(save_path.joinpath(f'saved_memories/rews_buf_{date}_reward={best_reward:.2f}_step={self.total_step}'), self.memory.rews_buf[:num_steps])

                    # save agent
                    with open(save_path.joinpath(f'saved_agents/DDPGAgent_{date}_reward={best_reward:.2f}_step={self.total_step}.pkl'), 'wb') as agent_file:
